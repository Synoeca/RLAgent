...
class DDQNSolver(nn.Module):
    def __init__(self, output_dim):
        super().__init__()
        self.online = nn.Sequential( # online
        ...
        self.target = copy.deepcopy(self.online) # target
...
class DDQNAgent:
    def __init__(self, action_dim, save_directory):
        ...
        self.exploration_rate = 1.0 # Initial exploration rate for epsilon-greedy strategy.
        self.exploration_rate_decay = 0.99 # Decay factor for the exploration rate.
...
load_checkpoint = "checkpoint_460.pth"
agent = DDQNAgent(action_dim=env.action_space.n, save_directory=save_directory)
episode = 0

if load_checkpoint is not None and os.path.exists(os.path.join(save_directory, load_checkpoint)):
